...
class TestSegmentation:
    ...
    def test_srg2d_u8(self):
        ...
        assert np.unique(seg_result).size <= np.unique(
            self.seeds2d).size, "Number of segments should not exceed number of seeds"
        assert np.all(seg_result >= np.amin(self.seeds2d)) and np.all(
            seg_result <= np.amax(self.seeds2d)), "Segment labels should be between 0 and 4"
    ...
    def test_srg3d_u8(self):
        ...
        assert seg_result.dtype == np.uint8, "Segmented image should be of type uint8"
        assert np.unique(seg_result).size <= np.unique(
            self.seeds3d).size, "Number of segments should not exceed number of seeds"
        assert np.all(seg_result >= np.amin(self.seeds3d)) and np.all(
            seg_result <= np.amax(self.seeds3d)), "Segment labels should be between 0 and 4"
    ...
    def test_srm2d_u16(self):
        ...
        seg_true = np.array([[64498, 20863, 20863, 20863, 54049, 54049, 54049, 20863, 65531, 20863],
                            [64498, 20863, 20863, 20863, 54049,
                                54049, 54049, 20863, 20863, 20863],
                            [20863, 20863, 50250, 50250, 20863,
                                20863, 20863, 59446, 59446, 20863],
                            [20863, 20863, 20863, 20863, 50509,
                                20863, 20863, 20863, 20863, 20863],
                            [20863, 20863, 20863, 50509, 50509,
                                20863, 20863, 20863, 20863, 20863],
                            [20863, 20863, 54395, 20863, 50509,
                                20863, 20863, 41028, 41028, 41028],
                            [20863, 20863, 54395, 20863, 20863,
                                20863, 20863, 20863, 41028, 41028],
                            [60526, 20863, 54395, 20863, 61821,
                                61821, 20863, 20863, 41028, 41028],
                            [60526, 20863, 20863, 20863, 20863,
                                61821, 41028, 41028, 41028, 41028],
    ...
    def test_srm2d_u32(self):
        ...
        seg_true = np.array([[4159147423, 2359534120, 1120177789, 1323907596, 4023327551, 2325579152,  3955417615,  865515529, 4294631142,  525965849],
                            [4294967295,  678763205, 1340885080,  271303589, 3921462647,
                                4159147423,  2868858639, 1561592372, 2478376508, 1255997660],
                            [933425465,   84551265, 2716061284, 3870530195,   84551265,
                                1239020177,   763650625, 3904485163, 3887507679,  508988365],
                            [3191430835, 1748344696, 1425772500, 1289952628, 3955417615,
                                1765322180,  1188087725, 1289952628,  152461201, 2800948703],
                            [1188087725, 1748344696, 1901142052, 3785642775, 3055610963,
                                2070916892,  2393489088,  542943333,  967380433,  729695657],
                            [2970723543, 2376511604, 3191430835, 1001335401, 2444421540,
                                1357862564,  1069245337, 3700755355, 3089565931, 2478376508],
                            [373168493, 2172781796, 3768665291, 1188087725, 1239020177,
                                2070916892,  2325579152, 1289952628, 2834903671, 2631173864],
                            [4023327551, 1510659920, 3734710323,  118506233, 4159147423,
                                4210079875,    67573781,  695740689, 1493682436, 2970723543],
                            [4294967295, 2274646700,  678763205,   33618813, 2104871860,
                                3785642775,  1918119536, 3463070579, 2138826828, 2902813607],
    ...
    def test_srm3d_u16(self):
        ...
        seg_true = np.array([[46067, 17176, 17176, 17176, 46067, 46067, 46067, 17176, 17176, 17176],
                            [46067, 17176, 17176, 17176, 46067,
                                46067, 46067, 17176, 17176, 17176],
                             [17176, 17176, 46067, 46067, 17176,
                                 17176, 17176, 46067, 46067, 17176],
                            [46067, 46067, 46067, 46067, 46067,
                                17176, 17176, 17176, 17176, 17176],
                            [46067, 46067, 46067, 46067, 46067,
                                17176, 17176, 17176, 17176, 17176],
                            [46067, 46067, 46067, 17176, 46067,
                                17176, 17176, 46067, 46067, 46067],
                            [5397, 46067, 46067, 17176, 17176,
                                17176, 17176, 17176, 46067, 46067],
                            [46067, 46067, 46067, 17176, 46067,
                                46067, 17176, 17176, 46067, 46067],
                            [46067, 46067, 17176, 17176, 46067,
                                46067, 17176, 46067, 46067, 46067],
    ...
    def test_srm3d_u32(self):
        ...
        seg_true = np.array([[4126537205, 2341178251, 1111638594, 1313754702, 3991793133, 2307492233, 3924421097,  858993459,          0,  522133279],
                            [4261281277,  673720360, 1330597711,  269488144, 3890735079,
                                4126537205, 2846468521, 1549556828, 2459079314, 1246382666],
                            [926365495,   84215045, 2694881440, 3840206052,   84215045,
                                1229539657,  757935405, 3873892070, 3857049061,  505290270],
                            [3166485692, 1734829927, 1414812756, 1280068684, 3924421097,
                                1751672936, 1179010630, 1280068684,  151587081, 2779096485],
                            [1179010630, 1734829927, 1886417008, 3755991007, 3031741620,
                                2054847098, 2374864269,  538976288,  960051513,  724249387],
                            [2947526575, 2358021260, 3166485692,  993737531, 2425393296,
                                1347440720, 1061109567, 3671775962, 3065427638, 2459079314],
                            [370546198, 2155905152, 3739147998, 1179010630, 1229539657,
                                2054847098, 2307492233, 1280068684, 2812782503, 2610666395],
                            [3991793133, 1499027801, 3705461980,  117901063, 4126537205,
                                4177066232,   67372036,  690563369, 1482184792, 2947526575],
                            [4261281277, 2256963206,  673720360,   33686018, 2088533116,
                                3755991007, 1903260017, 3435973836, 2122219134, 2880154539],
