...
class TestMetrics:
    def setup_class(self):
        ...
        self.spheres = ps.generators.overlapping_spheres(
            shape=[100, 100, 100], r=10, porosity=0.6)

    def test_edt_ball(self):
        euclidean_distance = dpm_tools.metrics.edt(self.ball)
        assert_allclose([euclidean_distance.min(),
                        euclidean_distance.max()], [0.0, 10.049875])

    def test_sdt_ball(self):
        signed_distance = dpm_tools.metrics.sdt(self.ball.astype(np.int16))
        assert_allclose(
            [signed_distance.min(), signed_distance.max()], [-7.5498343, 10.049875])

    def test_mis_ball(self):
        inscribed_sphere = dpm_tools.metrics.mis(self.ball)
        assert_allclose(
            [inscribed_sphere.min(), inscribed_sphere.max()], [0.0, 10.049875])

    def test_slicewise_edt_ball(self):
        slicewise_euclidean_distance = dpm_tools.metrics.slicewise_edt(
            self.ball)
        ...
        assert_allclose(
            np.max(slicewise_euclidean_distance, axis=(0, 1)), max_vals)

    # @pytest.mark.skip(reason="Passes locally but fails on github actions")
    # def test_slicewise_mis_ball(self):
    #     slicewise_inscribed_spheres = dpm_tools.metrics.slicewise_mis(
    #         self.ball)
    #     max_vals = np.array([1, 4, 6, 7, 7, 8, 9, 9, 9, 10,
    #                         10, 10, 9, 9, 9, 8, 7, 7, 6, 4, 1, 1])
    ...
    #         self.ball, boundary='l', detrend=False)
    #     tofr_trended = dpm_tools.metrics.time_of_flight(
    #         self.ball, boundary='r', detrend=False)

    #     assert_allclose([tofl_trended.max(), tofr_trended.max()], [19.5, 19.5])

    #     tofl_detrended = dpm_tools.metrics.time_of_flight(
    #         self.ball, boundary='l', detrend=True)
    #     tofr_detrended = dpm_tools.metrics.time_of_flight(
    #         self.ball, boundary='r', detrend=True)
    ...
    def test_minkowski_2d_ball(self):
        ...
        area, perim, curv = dpm_tools.metrics.minkowski_functionals(
            self.ball[10])
        assert_allclose([area, perim, curv], (317.0, 65.973446, 1.0))

    def test_minkowski_3d_ball(self):
        volume, area, curv, euler = dpm_tools.metrics.minkowski_functionals(
            self.ball)
